...
2. $\vec{F}_{\text {cor }}=2 m\left(\vec{v}^{\prime} \times \vec{\omega}\right)$. $\vec{v}^{\prime}$ and $\vec{\omega}$ are continuously perpendicular to each other. {numref}`Figure {number} <1e2002/figure_3.png>` shows the direction of the resulting $F_{\text {cor }}$. So $F_{c o r}$ points as seen from $\bar{v}^{\prime}$ continuously to the left, giving $m$ a counter clockwise path.
