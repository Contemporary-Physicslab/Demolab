...
- Clamping material to fix the chain to the ladder (see [Diagram](#diagram)).  
  
## Safety   
 
 *  In the pictures the demonstrator is standing with his hands free. (We did so to make the picture clearer.) <b> When you stand on the ladder, hold yourself! </b>
     
  
## Presentation   
The ladder is placed and the chain with the yellow block is fixed to it. It hangs from such a height that the yellow block just touches the ground when it all hangs down (see [Diagram A](#diagram) in which the end with the yellow block is temporarily fixed to the ladder).

- First two small blocks of wood are kept fixed, squeezed between your thumb and forefinger. When you let them go they fall together towards the ground and touch it at the same time (see [Diagram B](#diagram) and {numref}`Figure {number} <1g1002/figure_1.png>`). 
