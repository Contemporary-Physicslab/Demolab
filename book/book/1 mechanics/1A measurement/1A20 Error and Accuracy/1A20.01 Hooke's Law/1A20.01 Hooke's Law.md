...
## Presentation   
One ruler is placed horizontal. The spring is fixed on it and can slide along the ruler when it is pulled by the spring balance (see {numref}`Figure {number} <demo1A2001/figure_0 >`Diagram A). The camera observes the position of the spring on the ruler and the force indicated by the spring balance, so all students can read the results obtained.

The second ruler stands vertical. The spring, with the scale fixed to it, hangs close to the ruler (see {numref}`Figure {number} <demo1A2001/figure_0 >` Diagram B). The camera observes again the position of the spring on the ruler.

We start with the horizontal arrangement. First, the equilibrium position ( $F=0$ ) is read. Then the spring is given a displacement $(x)$ of $5.0 \mathrm{~cm}$ from its equilibrium. The

corresponding force is read and the spring constant $(k)$ is calculated: $k=\frac{F}{x}$. (We find: $x=5.0 \mathrm{~cm} ; F=5.7 \mathrm{~N}$; so $k=114 \mathrm{~N} / \mathrm{m}$.) Also the error range ($u$) is determined:
...
in which $u(v)$ is determined by $v=\left|h_{o}-h_{m}\right|$ and $u(v)^{2}=u\left(h_{o}\right)^{2}+u\left(h_{m}\right)^{2}$. (We find: $u(\mathrm{~m})=0.1 \mathrm{~g}$; $u\left(h_{o}\right)=u\left(h_{m}\right)=0.1 \mathrm{~cm} ; u(g)=0.000001 \mathrm{~m} / \mathrm{s}^{2}$; so $\mathrm{u}(\mathrm{k})=3 \mathrm{~N} / \mathrm{m}$. Terms with $m$ and $g$ are neglectable in $u(\mathrm{k})$. )
