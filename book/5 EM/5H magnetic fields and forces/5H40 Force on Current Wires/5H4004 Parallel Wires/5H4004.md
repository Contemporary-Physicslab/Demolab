...
The wiring is set up in such a way that both wires conduct the current in the same direction (@5h4004_figure_1.png A). When switching on the current we see that the "loose" wire moves closer to the fixed wire.

Then the wiring is changed so that the two wires conduct the current in opposite directions (@5h4004_figure_1.png B). Switching on the current now shows that the "loose" wire is moving away from the fixed wire.    
...
Applying the rigth-hand rule shows the direction of this force between current $I_{1}(B)$ and $I_{2}$ (see @5h4004_figure_2.png).
