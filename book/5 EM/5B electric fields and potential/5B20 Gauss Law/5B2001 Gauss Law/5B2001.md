...
## Aim   
To introduce Gauss's law, the analogy between the velocity field of a fluid flow and the electric field is used.    
...
When this result is solidified, the demonstration is followed by a theoretical exercise of applying the same analysis to a fluid source that is allowed to expand outward uniformly. This will lead to a $1 /r^{2}$ dependence (see Explanation).

Once the analogy between $E=Q / 4 \pi \varepsilon_{0} t^{2}$ and $v=f / 4 \pi t^{2}$ is established, ask to identify the electric quantities that are analogues to the velocity, $v$ (the electric field $E$), and the fluid flow $f$ ( $Q / \varepsilon_{0}$). Since the electric field and velocity are both linear quantities, an equivalent of the continuity principle $f=v A$ must exist for electric fields. Given the analogies, students will find this electric continuity principle as $Q / \varepsilon_{0}=E A$ : Gauss's law! (The term $E A$ has then rightfully the meaning of "electric flux")
...
- Filling the flexible tubing of $50 \mathrm{~cm}$ is done by using two Hoffman clamps (see @5b2001_figure_2.png).
