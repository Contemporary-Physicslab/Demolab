...
- For a physical pendulum, the period $T$ is given by $T=\frac{2 \pi}{\sqrt{g}} \sqrt{\frac{I_{A}}{m s}}$.
    ...
    If the pendulum is a complete ring, then $s=R$ (see @3a1503_figure_1.png), $I_{A}=I_{C}+m R^{2}$ and $I_{c}=m R^{2}$. Then $T=\frac{2 \pi}{\sqrt{g}} \sqrt{2 R}$, so $I_{r}=2 R$.
...
. 
```
- If the pendulum is part of a complete ring, $I_{O}=m R^{2}$ (@3a1503_figure_2.png). Also $I_{O}=I_{C}+m(R-$ $s)^{2}$ (C is the center of mass) and $I_{A}=I_{C}+m s^{2}$. It follows that $I_{A}=2 m R s$ and $T=\frac{2 \pi}{\sqrt{g}} \sqrt{2 R}$. So again $I_{r}=2 R$.
