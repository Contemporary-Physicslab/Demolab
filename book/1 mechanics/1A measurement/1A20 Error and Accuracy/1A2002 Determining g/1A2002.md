...
Freek determining the value $g$, the acceleration due to gravity.
```
...
By using the code provided below, you can calculate the final value of $g$ with its associated uncertainty. 
...
The uncertainty is always given with only one significant figure. The final answer is presented with the same decimal digit (e.g., $9.8 \pm 0.3 \; \mathrm{m}/\mathrm{s}^2$).
