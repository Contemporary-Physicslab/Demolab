...
The recorded data are discussed now; a region of interest can be selected (see @1h1007_figure_1.png). It can be observed that the force-time relationship is a complicated one; nevertheless, Newton's third law is valid: at every moment in time, we see $F_{\text {paperclip }}=-F_{\text {magnet }}$.   
