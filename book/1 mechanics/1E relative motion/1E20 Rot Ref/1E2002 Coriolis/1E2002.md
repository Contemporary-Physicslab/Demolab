...
- Clamping material.
- Two overhead sheets (see @1e2002_figure_2.pngA, B).
...
```
 ...
 We also show prepared overhead sheets to elucidate the curvature more exactly. (See @1e2002_figure_2.png A, B and C. @1e2002_figure_2.pngC shows how the path on the rotating sheet/platform can be constructed: On sheet {numref}` {number} <1e2002_figure_2.pngA the "ball" moves with a constant speed; on sheet {numref}`{number} <1e2002_figure_2.pngC there is a rotation of $10^{\circ}$ for every $2 \mathrm{~cm}$ displacement on {numref}` {number} <1e2002_figure_2.pngA). Also, on these sheets, the counterclockwise curvature is clear.
